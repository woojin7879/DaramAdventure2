...
- 조건: **무기 최대 레벨 + 짝 패시브 1레벨 이상 보유**. 패시브 레벨은 요구하지 않는다.
...
- `src/codex.js`: `codexHTML(seen)`이 14종 카드를 렌더한다. 조건(기본 무기 최대 레벨 + 짝 패시브 Lv.1)은 항상 노출, 이름·태그·효과는 발견 후에만 보인다. 미발견 아이콘은 어두운 실루엣.
